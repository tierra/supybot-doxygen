...
# Name: Objectify from Gnosis_Utils 1.2.2
# Original Module: gnosis.xml.objectify
# Home-page: http://gnosis.cx/download/
# Author: Gnosis Software (David Mertz)
...
def createPyObj(s):
    ...
    klass = '_XO_'+s
    try:
        return objectify.__dict__[klass]()
    except:
        # Create an instance of the tag-named class
        exec ('class %s(objectify._XO_): pass' % klass)
        cl = locals()[klass]
        cl.PCDATA = None
        cl._seq = None
        objectify.__dict__[klass] = cl
        return cl()
